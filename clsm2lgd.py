...
class TrackLGDCalculator:
    ...
    # 读取 CLSM 文件的多变量数据并累加
    def read_clsm_var(self, file_path, variable_names=None):
        """
            读取GLDAS CLSMT025_D.nc文件中的指定变量并累加。
        :param file_path: str
            GLDAS CLSMT025_D.nc文件路径
        :param variable_names: list of str
            需要读取的变量名列表，例如 ['GWS_tavg', 'SoilMoi0_10cm_tavg']。
            如果为None，默认读取 ['GWS_tavg']。
        :return: dict: 包含以下内容的数据集
                - {combined_var_name}: 累加后的变量数据 [mm] (key名称由变量名拼接)
                - lat, lon, time
        """
        ...
        def safe_read_nc_variable(dataset, var_name, default_fill=-9999):
            ...
            if var_name not in dataset.variables:
                raise KeyError(f"变量 {var_name} 不在文件 {dataset.filepath()} 中。")
        ...
        try:
            ...
            for var_name in variable_names:
                data_slice = safe_read_nc_variable(dataset, var_name)

                if accumulated_data is None:
                    accumulated_data = data_slice
                else:
                    accumulated_data += data_slice  # 累加数据

            # 关闭文件
            dataset.close()

            # 生成组合变量名作为 Key
            combined_key = "+".join(variable_names)

            # 返回字典格式的数据
            result = {
                combined_key: accumulated_data,
                'lat': lat,
                'lon': lon,
                'time': time
            }
    ...
    def calculate_vars_mean_by_date_range(self, data_dir, start_date, end_date, variable_names=None):
        """
        批量读取指定日期范围内的所有数据并计算平均值。
        支持多个变量累加（例如计算总水储量 TWS = GWS + SoilMoi + ...）

        :param data_dir: str, 数据目录路径
        :param start_date: str, 开始日期 'YYYY-MM-DD'
        :param end_date: str, 结束日期 'YYYY-MM-DD'
        :param variable_names: list, 需要读取并累加的变量名列表，默认为 ['GWS_tavg']
        :return: dict, 包含平均值和坐标信息
        """
        ...
        # 生成数据的 Key 名称
        data_key = "+".join(variable_names)
        result_key = f"mean_{data_key}"  # 例如: mean_GWS_tavg+SoilMoi0_10cm_tavg

        current_date = start
        while current_date <= end:
            date_str = current_date.strftime("A%Y%m%d")
            # 注意：文件名通配符可能需要根据实际情况调整，这里假设格式固定
            filename = f"GLDAS_CLSM025_DA1_D.{date_str}.022.nc4"
            file_path = os.path.join(data_dir, filename)

            if os.path.exists(file_path):
                try:
                    # 传入变量列表
                    data_dict = self.read_clsm_var(file_path, variable_names=variable_names)
                    # 根据动态Key获取数据
                    all_data_list.append(data_dict[data_key])
        ...
        # 计算平均值
        print(f"开始计算变量 [{data_key}] 的格网平均值，共 {len(all_data_list)} 个文件...")
        data_stack = np.stack(all_data_list, axis=0)
        ...
        return {
            result_key: mean_data,  # 动态命名的Key
            'lat': lat,
            'lon': lon,
            'time_period': f"{start_date} to {end_date}",
            'num_files': len(all_data_list),
            'file_list': valid_files,
            'variables_used': variable_names
        }
