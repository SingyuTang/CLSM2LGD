...
# ================= 配置参数 =================
CONFIG = {
    'results_dir': r"./results_lgd/TWS_tavg",  # 上一步代码保存结果的文件夹
    ...
    'variable_name': 'CLSM Terrestrial water storage'   # 绘图标题和文件命名使用
}
...
def plot_combined_figure(data_list):
    ...
    main_title = f"{var_name}: Line-of-Sight Gravity Difference (LGD)\nAnalysis Period: {start_date} to {end_date}"
    fig.suptitle(main_title, fontsize=16, fontweight='bold', y=0.98)

    plt.tight_layout(rect=[0, 0, 1, 0.93])
    safe_var_name = var_name.replace(" ", "_")  # 把空格换成下划线
    output_filename = f'LGD_Plot_{safe_var_name}_{start_date}_{end_date}.png'
